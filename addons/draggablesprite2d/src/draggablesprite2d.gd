...
func snap_boundaries():
	...
	if not boundaries.encloses(rect):
		var left = boundaries.position.x - rect.position.x
		var right = (boundaries.position.x + boundaries.size.x) - (rect.position.x + rect.size.x)
		var top = boundaries.position.y - rect.position.y
		var bottom = (boundaries.position.y + boundaries.size.y) - (rect.position.y + rect.size.y)
		#print("l: %d, r: %d, t: %d, b: %d" % [left, right, top, bottom])
		if left > 0:
			position.x += left
		elif right < 0:
			position.x += right
